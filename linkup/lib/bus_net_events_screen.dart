...
class BusinessNetworkingScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Business & Networking Events"),
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            .collection('events')
            .where('category', isEqualTo: 'Business & Networking') // 🔥 Fixed: exact match
            .snapshots(),
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return const Center(child: Text("No Business & Networking events found."));
          }

          final events = snapshot.data!.docs
              .map((doc) {
            final data = doc.data() as Map<String, dynamic>;
            data['id'] = doc.id; // ✅ Add Firestore doc ID
            return data;
          })
              .where((event) =>
          (event['category'] as String?)?.trim().toLowerCase() ==
              'business & networking'.toLowerCase() &&
              event['date'] != null &&
          ...
          if (events.isEmpty) {
            return const Center(child: Text("No Business events in this month."));
          }
